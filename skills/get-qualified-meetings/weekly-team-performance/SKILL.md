...
description: "Generate your Weekly Team Performance dashboard from your La Growth Machine data: a head-of-sales cockpit that ranks each sender (identity) on reply rate, signups/meetings/deals and conversion, surfaces the hot leads going cold, and turns your best reps' campaigns into a coaching playbook for the rest. It pulls only YOUR live LGM data, computes one sortable all-KPI team table plus a value (€) layer, mines what your top campaigns do differently, classifies your real replies, and renders a live artifact you re-open every Monday. Use when a head of sales, SDR manager, GTM engineer or RevOps wants per-rep outbound performance, to know who converts best and why, which replies are unanswered, what's working to clone across the team, or a per-member reply-quality view. Triggers on: 'stats by member', 'team performance dashboard', 'per-rep outbound', 'who converts best', 'weekly team performance', 'sales team cockpit', 'coach my SDRs', 'who's generating replies', 'per-identity reply rate', 'objection handling by rep'. First run does a short setup (~10-15 min: connect LGM, pick your success event and how you track it, optional value layer); every run after just rebuilds the dashboard. Maintained by La Growth Machine."
---

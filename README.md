...
A library of Claude skills built to help GTM teams run their go-to-market motion — from sourcing lists and building campaigns to qualifying meetings and securing channels. Each skill works with your existing stack. Plug the **La Growth Machine MCP** on top to execute every step natively from Claude.
...
- **Single skills** — call one skill to run a focused task (build a Sales Navigator search, write a cold email, validate a list).
- **Use case skills** — complete workflows that chain several steps and act directly in La Growth Machine — e.g. generate a Sales Navigator search *and* import the audience into LGM in one flow.
...
| [campaign-challenger](skills/get-qualified-meetings/campaign-challenger/SKILL.md) | use-case | Challenge a campaign copy by benchmarking it against your existing campaigns |
| [campaign-impact-analyzer](skills/get-qualified-meetings/campaign-impact-analyzer/SKILL.md) | use-case | Rank campaigns by real revenue impact — cross-references LGM campaigns with CRM deals |
...
**Prerequisites:** [Claude Code](https://claude.com/product/claude-code) installed.

Each skill is a self-contained folder. Clone this repo, then copy the skill you want into your Claude skills directory:
...
| multichannel-campaign-builder | Sequence ready to copy into your tool | Create the campaign in LGM from the sequence |
...
| campaign-impact-analyzer | Works on pasted campaigns and deals | Cross-references LGM campaigns with CRM deals in one click |
